...
The owner asked that lease, seal, and commit steps no longer need their terminal, and chose: lanes for prototype, production Unity code, and rule/settings files; R2 as the highest rigor approved without them; a passphrase-protected SSH key; and an indefinite policy. This selects option B as the target and adds it to the near-term work. The detailed design is `docs/05-production/proposals/OWNER-POLICY-001/DESIGN.md` (revision 2). After an independent security review (`FIX_FIRST`, six P1 findings), the owner confirmed three choices against the reviewer's advice: all three lanes, an indefinite policy, and commits only to an `agent/*` branch. The design records them as accepted risks and adds the reviewer's technical corrections (strict command matching, a controlled commit script, concrete-path denials, an empty `allowed_commands` in policy mode, no serial number). R2 conflicts with ADR-001 D4 until the sandbox spike is accepted, so the verifier caps at R1. After three review rounds the owner adopted the reviewer's simpler variant on 2026-09-19: an unsigned, owner-written policy file plus a mandatory separate `git worktree` on an `agent/work` branch, instead of a signature; the design is now revision 4 and no signing key is needed (ADR-001 D8 is not required for this feature). This ADR stays `PROPOSED` until the owner accepts the reviewed design and both patches have been reviewed.
