...
- **DEBT-001/RISK-006 (P0 in code; operationally unblocked via confirmed workaround)**: `scripts/doctor.py:57-61` and `.claude/hooks/common.py:git_state()`/`git_worktree_clean()` (~lines 137-177) all call `subprocess.run(..., text=True, ...)` without `encoding="utf-8"`, breaking git-root path comparison on this repo's non-ASCII Windows path. **No agent can ever fix this in code** (`CONTROLLED_PATHS`).
  1. **Confirmed working, 2026-09-16**: human ran `$env:PYTHONUTF8=1; python scripts/doctor.py --require-claude` in their own PowerShell terminal → `RESULT: PASS` (was `FAIL` moments earlier without the env var, same terminal). No file edit needed. Use this to unblock lease activation: set `PYTHONUTF8=1` in the human's terminal before running `scripts/activate_lease.py`.
  2. Still untried: human manually adds `encoding="utf-8"` to the ~4 `subprocess.run` calls across the two files, then commits — this would make the fix permanent instead of depending on the env var being set every session. Recommended as the real fix, since (1) is a per-terminal workaround, not a closed debt.
  Do not re-litigate whether (1) works — it's confirmed. The remaining open question is only whether/when a human applies (2).
...
1. **Immediate**: with `PYTHONUTF8=1` set in the human's terminal (confirmed working, see above), the human runs `python scripts/activate_lease.py docs/05-production/tasks/TASK-SCHEMA-001.md` (human-only; still not yet attempted with the env var set — do this next and record the actual result).
