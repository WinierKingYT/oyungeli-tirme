...
- Design B as the target for removing routine steps, as two independently reviewed patches (see `docs/05-production/proposals/OWNER-POLICY-001/DESIGN.md`, revision 4: an owner-written policy file plus a mandatory separate agent worktree; no signing key). Until B exists and is accepted, agents keep asking the owner for lease, seal, and commit steps.
- Reject C. Consider E only if the owner explicitly accepts the weaker safety in writing.

## Owner input (2026-09-19)

The owner asked that lease, seal, and commit steps no longer need their terminal. Current decisions (design revision 4, `docs/05-production/proposals/OWNER-POLICY-001/DESIGN.md`): lanes for prototype, production Unity code, and engine rule files; R2 as the highest rigor approved without the owner (the verifier caps at R1 until the ADR-001 D4 sandbox is accepted); an indefinite policy; commits only to a standing `agent/work` branch in a separate `git worktree`, which the owner merges; and an unsigned, owner-written policy file with no signing key (ADR-001 D8 is not needed for this feature). History: an earlier choice of a passphrase-protected SSH key was dropped after three independent security reviews showed the signature added little security in this architecture; three choices (all three lanes, an indefinite policy, the branch model) were confirmed against the reviewer's advice and are recorded as accepted risks. The design also adopts the reviewer's technical corrections (strict command matching, a controlled commit script, concrete-path denials, an empty `allowed_commands` in policy mode, no serial number). This ADR stays `PROPOSED` until the owner accepts the reviewed design and both patches have been reviewed.
