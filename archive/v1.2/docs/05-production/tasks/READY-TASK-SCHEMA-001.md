...
task_sha256: d18941d0c5d1f54032438a4780a8214c70c3b2c7ae8a39faad8a000b3db2b6bb
...
**Note on the digest above:** it reflects the task contract after two post-review updates on 2026-09-16: (1) the anticipated DEBT-001 waiver note, and (2) a correction to that note after a real `activate_lease.py` run on this repository failed with `ERROR: AI Game Development OS must be installed at the Git repository root`, proving DEBT-001 also lives in `.claude/hooks/common.py:git_state()` and is a hard functional block, not a waivable item. Neither change altered this task's scope, acceptance criteria, or anything this receipt's review actually evaluated — both are factual/evidentiary corrections, not new design decisions — so no new review round was triggered, but the digest changed each time and this receipt was updated to match both times. **Current practical status: this task is contract-complete and technically `READY_FOR_IMPLEMENTATION`, but its lease cannot actually be activated on this machine until DEBT-001/RISK-006 (P0, see TECH-DEBT.md/RISK-REGISTER.md) is fixed by a human directly editing `scripts/doctor.py` and `.claude/hooks/common.py`.** Verify with `python scripts/task_digest.py docs/05-production/tasks/TASK-SCHEMA-001.md` before activating, and expect `activate_lease.py` to keep failing until that fix lands.
