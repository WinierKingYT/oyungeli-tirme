...
**Waiver recorded, 2026-09-16, superseded same day:** the project owner initially chose to waive the DEBT-001 trigger, on the (incorrect) understanding that it was not mechanically required by `scripts/activate_lease.py`. A subsequent real `python scripts/activate_lease.py` attempt on this repository failed with `ERROR: AI Game Development OS must be installed at the Git repository root` — proving DEBT-001's scope was under-stated: the same bug also exists in `.claude/hooks/common.py:git_state()`, which `activate_lease.py` calls directly and which every governed hook's `load_lease()` also depends on. This is a **hard functional block**, not a waivable due-diligence item — no waiver can make `activate_lease.py` succeed on this machine. DEBT-001/RISK-006 have been re-escalated to P0 in the backlog accordingly. This task remains `READY_FOR_IMPLEMENTATION` as a *contract* (nothing about its scope, acceptance criteria, or reviewed content changed), but its lease cannot actually be activated on this machine until a human fixes `encoding="utf-8"` handling in both `scripts/doctor.py` and `.claude/hooks/common.py` directly — neither file can ever be edited by an agent. The open sandbox spike (`SPIKE-SANDBOX-001`) concerns mandatory OS-level isolation for rigor tier **R2 and above** (ADR-001, D4); this task is **R1**, requires no sandboxed execution, and D4 does not apply to it — this part of the reconciliation was not disputed by either independent review.
